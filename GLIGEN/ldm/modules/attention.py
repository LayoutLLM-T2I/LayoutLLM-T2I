...
class RelationCrossAttention(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head):
        ...
        # self.avg_pool1 = nn.AvgPool2d(kernel_size=1, stride=1, padding=0)
        # self.avg_pool2 = nn.AvgPool1d(kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, relations, boxes, masks, h=None, w=None):
        b, _, c = x.shape
        hidden = rearrange(self.norm3(x), 'b (h w) c -> b h w c', h=h, w=w)
        mo = boxes.size(1)
        hidden = hidden.unsqueeze(1).repeat(1, mo, 1, 1, 1)
        obj_mask = torch.full(hidden.size(), fill_value=0).to(hidden)
        boxes_mask = torch.sum(masks, dim=-1).tolist()  # the number of boxes for each instance
        width = torch.full((b, mo), w).to(boxes)
        height = torch.full((b, mo), h).to(boxes)

        box_x_0 = (boxes[:, :, 0] * w).to(torch.int)
        box_y_0 = (boxes[:, :, 1] * h).to(torch.int)
        box_x_1 = torch.minimum(boxes[:, :, 2] * w, width).to(torch.int).tolist()  # x
        box_y_1 = torch.minimum(boxes[:, :, 3] * h, height).to(torch.int).tolist()  # y
        box_x_0 = box_x_0.tolist()
        box_y_0 = box_y_0.tolist()

        obj_features = torch.zeros((b, mo, c)).to(x)
        for k in range(b):
            for i in range(mo):
                left = box_x_0[k][i]
                right = box_x_1[k][i]
                top = box_y_0[k][i]
                bottom = box_y_1[k][i]
                if i < boxes_mask[k] and left != right and top != bottom:
                    obj_mask[k, i, top:bottom, left:right, :] = 1
                    _tmp_obj_features = hidden[k, i, top:bottom, left:right, :]
                    _tmp_obj_features = _tmp_obj_features.reshape(-1, c)
                    obj_features[k, i] = torch.mean(_tmp_obj_features, dim=0)
                else:
                    # the boxes are the pad one, there is no need to find the none-box feature.
                    break

        obj_features = obj_features + self.scale * torch.tanh(self.alpha_attn) * self.attn(self.norm1(obj_features),
                                                                                           relations, relations)
        # b, max_obj, c
        obj_features = obj_features + self.scale * torch.tanh(self.alpha_dense) * self.ff(self.norm2(obj_features))

        # b mo c h w
        obj_features = rearrange(obj_features.unsqueeze(3).repeat(1, 1, 1, w * h), "b mo c (h w) -> b mo h w c ", w=w,
                                 h=h)
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=False):
        ...
        if fuser_type == "gatedSA":
            ...
            assert False
        self.rela_fuse = RelationCrossAttention(query_dim, key_dim, value_dim, n_heads, d_head)
    ...
    def forward(self, x, context, objs, relations=None, boxes=None, masks=None, h=None, w=None):
        ...
        #     return checkpoint.checkpoint(self._forward, x, context, objs, relations, boxes, masks, h, w)
        # else:
